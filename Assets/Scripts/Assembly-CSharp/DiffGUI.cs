...
public class DiffGUI : MonoBehaviour
{
	...
	private void OnGUI()
	{
		...
		GUI.DrawTexture(new Rect(position2.x - (float)instr[_curInd].width * Defs.Coef, position.y, (float)instr[_curInd].width * Defs.Coef, (float)instr[_curInd].height * Defs.Coef), instr[_curInd]);
	}
}
