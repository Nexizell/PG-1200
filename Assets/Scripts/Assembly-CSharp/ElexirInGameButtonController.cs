...
public class ElexirInGameButtonController : MonoBehaviour
{
	...
	private void Awake()
	{
		string text = (GameConnect.isDaterRegim ? idForPriceInDaterRegim : myPotion.name);
		...
		if (GameConnect.isHunger || isKnifeMap)
		{
	...
	private void HandlePotionDisactivated(string obj)
	{
		if (obj.Equals(myPotion.name))
		{
			...
			int @int = Storager.getInt(GameConnect.isDaterRegim ? GearManager.HolderQuantityForID(idForPriceInDaterRegim) : obj);
	...
	{
		int @int = Storager.getInt(GameConnect.isDaterRegim ? GearManager.HolderQuantityForID(idForPriceInDaterRegim) : myPotion.name);
