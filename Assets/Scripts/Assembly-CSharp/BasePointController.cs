...
public class BasePointController : MonoBehaviour
{
	...
	private void Update()
	{
		...
		{
			Color color = ((WeaponManager.sharedManager.myNetworkStartTable.myCommand == 0) ? Color.gray : (((captureCounter > 0f && WeaponManager.sharedManager.myNetworkStartTable.myCommand == 1) || (captureCounter < 0f && WeaponManager.sharedManager.myNetworkStartTable.myCommand == 2)) ? Color.blue : Color.red));
		...
		if (captureCounter > 99.9f)
		{
			...
			if (WeaponManager.sharedManager.myPlayerMoveC != null && !isSendMessageCaptureBlue)
			{
				...
				{
					QuestMediator.NotifyCapture(GameConnect.GameMode.CapturePoints);
					if (num == 1)
					{
		...
		if (myPlayerOnPoint && InGameGUI.sharedInGameGUI != null)
		{
			...
			InGameGUI.sharedInGameGUI.captureBarSprite.color = (flag2 ? blueTeamColor : redTeamColor);
			InGameGUI.sharedInGameGUI.teamColorSprite.color = (flag2 ? blueCaptureColor : redCaptureColor);
		}
	...
	{
		if (Defs.isMulti && GameConnect.isCapturePoints && other.transform.parent != null && other.transform.parent.gameObject.CompareTag("Player"))
		{
	...
	{
		if (Defs.isMulti && GameConnect.isCapturePoints)
		{
