...
public static class GearManager
{
	...
	private static bool ItemIsGear(string item, string gear)
	{
		...
		int result;
		if (!(item == gear) && (!item.StartsWith(text) || item.Length <= text.Length || !int.TryParse(item[text.Length].ToString() ?? "", out result)))
		{
			if (item.StartsWith(text2) && item.Length > text2.Length)
			{
				return int.TryParse(item[text2.Length].ToString() ?? "", out result);
			}
			return false;
		}
		return true;
