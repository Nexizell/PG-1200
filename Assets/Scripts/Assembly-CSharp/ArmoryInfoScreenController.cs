...
public class ArmoryInfoScreenController : MonoBehaviour
{
	...
	[SerializeField]
	protected internal GameObject hintsContainer;
	...
	public void OnSelectItem(ItemPreviewInArmoryInfoScreen selectedItem, ShopNGUIController.CategoryNames category)
	{
		...
		{
			currentUpgradeWindows.SetUpgrade(selectedItem.numUpgrade, (text != null) ? actualUpgrades.IndexOf(text) : (-1));
		}
	...
	public void SetItem(ShopNGUIController.ShopItem item)
	{
		...
		{
			List<string> item4 = GadgetsInfo.Upgrades[item.Id];
		...
		LoadPreviewCarousel(item.Category, item.Id, list, num, (selectedItemController != null) ? selectedItemController.id : string.Empty);
	...
	public void LoadPreviewCarousel(ShopNGUIController.CategoryNames category, string itemId, List<string> _availableTags, int startIndexUpgrade, string selectedItemID)
	{
		...
		for (int i = 0; i < _availableTags.Count; i++)
		{
			...
			float num = ((currentId == selectedItemID) ? ItemPreviewInArmoryInfoScreen.maxScale : ItemPreviewInArmoryInfoScreen.minScale);
