...
public sealed class RatingSystem : MonoBehaviour
{
	...
	public struct RatingChange
	{
		...
		public float oldRatingAmount
		{
			get
			{
				...
				float num2 = instance.leagueRatings[Mathf.Clamp(GetOldLeagueIndex() + 3, 0, instance.leagueRatings.Length - 1)];
				float a = ((oldRating > newRating) ? 0.03f : 0.015f);
				if (oldRating == 0)
				{
					a = 0f;
				}
		...
		public float newRatingAmount
		{
			get
			{
				float num = instance.leagueRatings[Mathf.Clamp(GetOldLeagueIndex(), 0, instance.leagueRatings.Length - 1)];
				float num2 = instance.leagueRatings[Mathf.Clamp(GetOldLeagueIndex() + 3, 0, instance.leagueRatings.Length - 1)];
				float a = ((newRating > oldRating) ? 0.03f : 0.015f);
		...
		public int maxRating
		{
			get
			{
				return instance.MaxRatingInLeague(oldLeague);
			}
		}
		...
		{
			return (int)newLeague * 3;
		}

		private int GetOldLeagueIndex()
		{
			return (int)oldLeague * 3;
		}
	}
	...
	private int[] leagueNCoefs = new int[16]
	{
		50, 50, 50, 100, 100, 100, 150, 150, 150, 200,
		200, 200, 250, 300, 350, 1000000
	};

	private SaltedIntArray leagueRatings = new SaltedIntArray(new int[16]
	{
	...
	public static RatingSystem instance
	{
		get
		{
			if (_instance == null)
			{
				GameObject obj = new GameObject("RatingSystem");
				UnityEngine.Object.DontDestroyOnLoad(obj);
				_instance = obj.AddComponent<RatingSystem>();
			}
			return _instance;
		}
	}
	...
	private int form_n_coef
	{
		get
		{
			return leagueNCoefs[Mathf.Min((int)currentLeague * 3 + currentDivision, leagueNCoefs.Length - 1)];
		}
	}
	...
	public int positiveRating
	{
		get
		{
			...
			{
				return Storager.getInt("RatingPositive");
			}
			return 0;
		}
		set
		{
			Storager.setInt("RatingPositive", value);
		}
	}
	...
	public int positiveRatingLocal
	{
		get
		{
			if (!Storager.hasKey("RatingPositiveLocal"))
			{
				return 0;
			}
			return Storager.getInt("RatingPositiveLocal");
		}
		set
		{
			Storager.setInt("RatingPositiveLocal", value);
		}
	}
	...
	public int negativeRating
	{
		get
		{
			...
			{
				return Storager.getInt("RatingNegative");
			}
			return 0;
		}
		set
		{
			Storager.setInt("RatingNegative", value);
		}
	}
	...
	private void AddToRating(int rating)
	{
		...
		SaveValues();
		Debug.Log(string.Format("<color=yellow>Add {0} rating.</color>", new object[1] { rating }));
		Debug.Log(string.Format("<color=yellow>I'm in {0} league, division: {1}. Rating: {2}</color>", new object[3]
		{
	...
	{
		int num = ((matchKillrate <= form_kd_top) ? Mathf.RoundToInt(matchKillrate * form_kd_b - form_kd_a) : Mathf.RoundToInt(form_kd_b));
	...
	public int GetRatingForRooms()
	{
		return Mathf.Clamp(currentRating, RatingNeededForLeague(currentLeague) + form_n_coef / 2, MaxRatingInLeague(currentLeague) - form_n_coef / 2);
	}

	public int GetLowBorderForRooms()
	{
		return Mathf.Max(0, GetRatingForRooms() - form_n_coef);
	}

	public int GetHighBorderForRooms()
	{
		return GetRatingForRooms() + form_n_coef;
	}
	...
	public void BackupLastRatingTake()
	{
		if (lastRatingChange.Value < 0)
		{
			...
			UpdateLeague(true);
			SaveValues();
			Debug.Log(string.Format("<color=yellow>Rating backup: {0} rating.</color>", new object[1] { lastRatingChange.Value }));
			Debug.Log(string.Format("<color=yellow>I'm in {0} league, division: {1}. Rating: {2}</color>", new object[3]
			{
				currentLeague.ToString(),
				3 - currentDivision,
	...
	private void UpdateLeague(bool up)
	{
		...
		for (int i = (up ? num2 : 0); i < (up ? leagueRatings.Length : (num2 + 1)); i++)
		{
	...
	{
		Dictionary<string, object> dictionary = Json.Deserialize(Storager.getString("RatingSystem")) as Dictionary<string, object>;
		if (dictionary != null)
		{
	...
	private void SaveValues()
	{
		...
		Storager.setString("RatingSystem", Json.Serialize(dictionary));
	}
	...
	{
		Dictionary<string, object> dictionary = Json.Deserialize(Storager.getString("rSCKeyV3")) as Dictionary<string, object>;
		if (dictionary == null)
		{
			return;
		}
		...
		if (dictionary.ContainsKey("leagueNCoefs"))
		{
			Defs.useRatingLobbySystem = true;
			List<object> list4 = dictionary["leagueNCoefs"] as List<object>;
			for (int l = 0; l < list4.Count && l < leagueNCoefs.Length; l++)
			{
				leagueNCoefs[l] = Convert.ToInt32(list4[l]);
			}
		}
		else
		{
			Defs.useRatingLobbySystem = false;
		}
	}
}
