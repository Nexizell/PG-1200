...
public sealed class BonusItem : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (isTimeBonus && PhotonNetwork.isMasterClient && Defs.isInet && PhotonNetwork.time > expireTime)
		{
			BonusController.sharedController.RemoveBonus(myIndex);
		}
		else
		{
			if (player == null || playerMoveC == null || playerMoveC.isKilled || !(Vector3.SqrMagnitude(base.transform.position - player.transform.position) < 4f))
			{
				return;
			}
			bool flag = false;
			...
			if (flag)
			{
				...
				{
					playerMoveC.myNetworkStartTable.AddToScore(2);
				}
				if (Defs.isSoundFX)
				{
