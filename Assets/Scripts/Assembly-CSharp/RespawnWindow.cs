...
public sealed class RespawnWindow : MonoBehaviour
{
	...
	private void FillItemsToBuy(KillerInfo killerInfo)
	{
		try
		{
			List<string> list = ((BuildSettings.BuildTargetPlatform == RuntimePlatform.MetroPlayerX64) ? new List<string>() : GetWeaponsForBuy());
			string itemTag = ((list.Count > 0) ? list[0] : null);
			string text = ((BuildSettings.BuildTargetPlatform == RuntimePlatform.MetroPlayerX64) ? null : GetArmorForBuy());
			if (string.IsNullOrEmpty(text))
			{
				text = ((list.Count > 1) ? list[1] : null);
			}
	...
	public void OnItemToBuyClick(RespawnWindowItemToBuy itemToBuy)
	{
		...
		ShopNGUIController.TryToBuy(base.gameObject, itemToBuy.itemPrice, delegate
		{
			...
			if (GearManager.IsItemGear(itemToBuy.itemTag))
			{
				...
				{
					int b = Defs2.MaxGrenadeCount - Storager.getInt(itemToBuy.itemTag);
					num = Mathf.Min(num, b);
				}
			}
			ShopNGUIController.ProvideItem(itemCategory, ShopNGUIController.IsWeaponCategory(itemCategory) ? WeaponManager.FirstUnboughtOrForOurTier(itemToBuy.itemTag) : itemToBuy.itemTag, num, false, 0, delegate(string item)
			{
			...
			{
				string shopId = "";
				string itemNameNonLocalized = ItemDb.GetItemNameNonLocalized(WeaponManager.LastBoughtTag(itemToBuy.itemTag) ?? WeaponManager.FirstUnboughtTag(itemToBuy.itemTag), shopId, itemCategory);
	...
	private void FillStats(KillerInfo killerInfo)
	{
		...
		if (armorCountFor > 0)
		{
			...
			armorCountLabel.text = string.Format("{0}/{1}", new object[2]
			{
				Mathf.Min(armorCountFor, killerInfo.armorValue),
				armorCountFor
			});
		...
		healthCountLabel.text = string.Format("{0}/{1}", new object[2]
		{
			killerInfo.healthValue,
			ExperienceController.HealthByLevel[killerInfo.rank]
		});
	}
	...
	{
		if (ExchangeWindow.IsOpened && _buyRequested)
		{
			_buyRequested = false;
			ExchangeWindow.Hide();
		}
