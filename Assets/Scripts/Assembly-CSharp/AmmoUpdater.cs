...
public sealed class AmmoUpdater : MonoBehaviour
{
	...
	private void Update()
	{
		if (WeaponManager.sharedManager != null && WeaponManager.sharedManager.currentWeaponSounds != null)
		{
			...
			{
				_label.text = (component.isShotMelee ? FormatMeleeAmmoLabel(weapon.currentAmmoInClip, weapon.currentAmmoInBackpack) : FormatShootingAmmoLabel(weapon.currentAmmoInClip, weapon.currentAmmoInBackpack, component.ammoInClip));
	...
	private string FormatShootingAmmoLabel(int currentAmmoInClip, int currentAmmoInBackpack, int fullClipAmmo)
	{
		...
		{
			string value = currentAmmoInClip + "/" + (GameConnect.isSpleef ? fullClipAmmo : currentAmmoInBackpack);
