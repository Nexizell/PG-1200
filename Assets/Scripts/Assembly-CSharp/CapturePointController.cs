...
public class CapturePointController : MonoBehaviour
{
	...
	private void Start()
	{
		if (!GameConnect.isCapturePoints || !Defs.isMulti)
		{
		...
		{
			int num = (int)PhotonNetwork.room.customProperties[GameConnect.maxKillProperty];
			if (num == 3)
			{
				speedScoreAdd = 3f;
			}
	...
	public void EndMatch()
	{
		...
		if (!TimeGameController.sharedController.isEndMatch && !isEndMatch)
		{
			...
			{
				WeaponManager.sharedManager.myNetworkStartTable.win("", commandWin, Mathf.RoundToInt(scoreBlue), Mathf.RoundToInt(scoreRed));
			}
		}
		else
		{
