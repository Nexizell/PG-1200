...
public sealed class TurretController_Minigun : TurretController_Tower
{
	...
	protected override void Shot()
	{
		...
		{
			bool num = hitInfo.collider.transform.root != null && hitInfo.collider.transform.root.gameObject.Equals(WeaponManager.sharedManager.myPlayer);
			bool flag = false;
			if (num || HitIDestructible(hitInfo.collider.gameObject))
			{
				flag = true;
			}
