...
namespace I2.Loc
{
	...
	public class Localize : MonoBehaviour
	{
		...
		{
			string secondary = ((mTarget_UILabel.ambigiousFont != null) ? mTarget_UILabel.ambigiousFont.name : string.Empty);
		...
		{
			string secondary = ((mTarget_UISprite.atlas != null) ? mTarget_UISprite.atlas.name : string.Empty);
		...
		{
			string secondary = ((mTarget_GUIText.font != null) ? mTarget_GUIText.font.name : string.Empty);
		...
		private void DoLocalize_Child(string MainTranslation, string SecondaryTranslation)
		{
			if (!mTarget_Child || !(mTarget_Child.name == MainTranslation))
			{
				...
				if ((bool)gameObject2)
				{
					...
					Transform obj = mTarget_Child.transform;
					Transform transform = (gameObject ? gameObject.transform : gameObject2.transform);
