...
{
	[CompilerGenerated]
	internal sealed class _003CWaitRequestResponse_003Ed__47 : IEnumerator<object>, IEnumerator, IDisposable
	{
		...
		[DebuggerHidden]
		public _003CWaitRequestResponse_003Ed__47(int _003C_003E1__state)
		{
		...
		private bool MoveNext()
		{
			...
			{
				UnityEngine.Debug.LogWarningFormat("Request failed: {0}", request.error);
				return false;
			}
			UnityEngine.Debug.LogFormat("Request succeeded: {0}", request.text);
			return false;
		}
	...
	[SerializeField]
	protected internal GameObject _callFriendButtonHolder;

	[SerializeField]
	protected internal GameObject friendCallLabel;

	[SerializeField]
	protected internal GameObject friendCalledLabel;
	...
	public void HandleCallFriend()
	{
		...
		if (Application.isEditor)
		{
			UnityEngine.Debug.LogFormat("`HandleCallFriend()`: `{0}`", Encoding.UTF8.GetString(wWWForm.data, 0, wWWForm.data.Length));
		}
	...
	{
		return string.Format("[b]{0} {1}[/b]", new object[2] { level, name });
	}
	...
	private void SetStatusLabelPlayerBusy()
	{
		...
		detailInfo.text = string.Format("[ff0000]{0}[-]", new object[1] { LocalizationStore.Get("Key_0576") });
	...
	private void SetStatusLabelPlayerPlaying(string gameModeName, string mapName)
	{
		...
		{
			detailInfo.text = string.Format("[00aeff]{0}[-]", new object[1] { gameModeName });
		}
		else
		{
			detailInfo.text = string.Format("[77ef00]{0}: {1}[-]", new object[2] { gameModeName, mapName });
		}
		RefreshCallFriendButton(false);
	}
	...
	private void SetStatusLabelFindOrigin(FriendsController.PossiblleOrigin findOrigin)
	{
		...
		case FriendsController.PossiblleOrigin.Local:
			detailInfo.text = string.Format("[ffe400]{0}[-]", new object[1] { LocalizationStore.Get("Key_1569") });
			inYourNetworkIcon.SetActive(true);
			break;
		case FriendsController.PossiblleOrigin.Facebook:
			detailInfo.text = string.Format("[00aeff]{0}[-]", new object[1] { LocalizationStore.Get("Key_1570") });
			break;
		case FriendsController.PossiblleOrigin.RandomPlayer:
			detailInfo.text = string.Format("[77ef00]{0}[-]", new object[1] { LocalizationStore.Get("Key_1571") });
			break;
		}
		RefreshCallFriendButton(false);
	}
	...
	private void SetOfflineStatePreview()
	{
		...
		BattleInviteListener.Instance.CallToFriendEnabled(id);
		OnlineCodeStatus = 3;
	...
	private void UpdateOnline()
	{
		...
		FriendsController.ResultParseOnlineData resultParseOnlineData = FriendsController.ParseOnlineData(FriendsController.sharedController.onlineInfo[id]);
	...
	public void OnClickAddFriend()
	{
		if (!string.IsNullOrEmpty(id))
		{
			...
			string value = (flag ? "Search" : string.Format("Find Friends: {0}", new object[1] { (FriendsController.PossiblleOrigin)FindOrigin }));
			...
			FriendsController.SendFriendshipRequest(id, dictionary, CallbackFriendAddRequest);
		}
	}
