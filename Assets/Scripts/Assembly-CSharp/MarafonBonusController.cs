...
public sealed class MarafonBonusController
{
	...
	public MarafonBonusController()
	{
		...
		_currentBonusIndex = Storager.getInt(Defs.NextMarafonBonusIndex);
	}
	...
	{
		int num = (int)((PremiumAccountController.Instance != null) ? PremiumAccountController.Instance.GetCurrentAccount() : PremiumAccountController.AccountType.None);
		day--;
	...
	{
		int val = Storager.getInt(gearId) + addCount;
		Storager.setInt(gearId, val);
	}
	...
	private void TakeBonusPlayer(int indexBonus)
	{
		if (BonusItems.Count != 0)
		{
			...
			{
				Storager.setInt(Defs.NextMarafonBonusIndex, indexBonus + 1);
			}
			_currentBonusIndex.Value = Storager.getInt(Defs.NextMarafonBonusIndex);
	...
	{
		return _currentBonusIndex.Value != -1;
	}

	public bool IsNeedShow()
	{
		return Storager.getInt(Defs.NeedTakeMarathonBonus) == 1;
	}
	...
	{
		return BonusItems[_currentBonusIndex.Value].type == BonusItemType.TemporaryWeapon;
	}
