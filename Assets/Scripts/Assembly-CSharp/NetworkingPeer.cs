...
internal class NetworkingPeer : LoadBalancingPeer, IPhotonPeerListener
{
			...
			if (Server == ServerConnection.NameServer || Server == ServerConnection.MasterServer)
			{
				if (operationResponse.Parameters.ContainsKey(225))
				{
					...
					if (!string.IsNullOrEmpty(text))
					{
						...
						if (PhotonNetwork.logLevel >= PhotonLogLevel.Informational)
						{
							DebugReturn(DebugLevel.INFO, string.Format("Received your UserID from server. Updating local value to: {0}", new object[1] { text }));
						}
					}
				}
	...
	public void OnStatusChanged(StatusCode statusCode)
	{
		...
		case StatusCode.QueueIncomingUnreliableWarning:
			Debug.Log(string.Concat(statusCode, ". This client buffers many incoming messages. This is OK temporarily. With lots of these warnings, check if you send too much or execute messages too slow. ", PhotonNetwork.isMessageQueueRunning ? "" : "Your isMessageQueueRunning is false. This can cause the issue temporarily."));
			break;
		default:
	...
	public void OnEvent(EventData photonEvent)
	{
		...
		case 209:
		{
			...
			if (PhotonNetwork.logLevel == PhotonLogLevel.Informational)
			{
				Debug.Log(string.Concat("Ev OwnershipRequest ", photonView.ownershipTransfer, ". ActorNr: ", num, " takes from: ", num4, ". Current owner: ", photonView.ownerId, " isOwnerActive: ", photonView.isOwnerActive.ToString(), ". MasterClient: ", mMasterClientId, ". This client's player: ", PhotonNetwork.player.ToStringFull()));
			}
		...
		case 210:
		{
			...
			Debug.Log("Ev OwnershipTransfer. ViewID " + array5[0] + " to: " + array5[1] + " Time: " + Environment.TickCount % 1000);
			int viewID = array5[0];
			int ownerId = array5[1];
		...
		case 229:
		{
			...
			foreach (object key2 in hashtable3.Keys)
			{
				...
				RoomInfo roomInfo = new RoomInfo(text2, (ExitGames.Client.Photon.Hashtable)hashtable3[key2]);
		...
		case 206:
		{
			...
			byte b2 = b;
			while (b2 - b < hashtable2.Count - num6)
			{
	...
	protected internal void ExecuteRpc(ExitGames.Client.Photon.Hashtable rpcData, PhotonPlayer sender)
	{
		...
		{
			PhotonTrafficStatistic.AddIncomingRPC(text, array);
		}
	...
	internal void RPC(PhotonView view, string methodName, PhotonTargets target, PhotonPlayer player, bool encrypt, params object[] parameters)
	{
		...
		if (Application.isEditor)
		{
			PhotonTrafficStatistic.AddOutcomingRPC(methodName, parameters);
		}
		if (player != null)
		{
	...
	public void SetReceivingEnabled(int[] enableGroups, int[] disableGroups)
	{
		...
		OpChangeGroups((list2.Count > 0) ? list2.ToArray() : null, (list.Count > 0) ? list.ToArray() : null);
	}
