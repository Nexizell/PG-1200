...
public class TryGunScreenController : MonoBehaviour
{
	...
	public bool ExpiredTryGun
	{
		...
		set
		{
			try
			{
				...
				if (value)
				{
					...
					try
					{
						foreach (UILabel discountLabel in discountLabels)
						{
							...
							discountLabel.text = string.Format(LocalizationStore.Get("Key_1996"), new object[2]
							{
								Mathf.RoundToInt(WeaponManager.TryGunPromoDuration() / 60f),
								ShopNGUIController.DiscountFor(ItemTag, out onlyServerDiscount)
							});
						}
					}
					catch (Exception ex)
				...
				int num = (ABTestController.useBuffSystem ? BuffSystem.instance.GetRoundsForGun() : KillRateCheck.instance.GetRoundsForGun());
				foreach (UILabel numberOfMatchesLabel in numberOfMatchesLabels)
				{
					numberOfMatchesLabel.text = string.Format(LocalizationStore.Get("Key_1995"), new object[1] { num });
				}
			}
			catch (Exception ex2)
			{
	...
	public void HandleBuy()
	{
		...
		ShopNGUIController.TryToBuy(base.gameObject, price, delegate
		{
			...
			{
				string shopId = "";
				string itemNameNonLocalized = ItemDb.GetItemNameNonLocalized(WeaponManager.LastBoughtTag(ItemTag) ?? WeaponManager.FirstUnboughtTag(ItemTag), shopId, category);
