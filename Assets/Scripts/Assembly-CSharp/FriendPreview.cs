...
public sealed class FriendPreview : MonoBehaviour
{
	...
	private void UpdateInfo()
	{
		...
		foreach (Dictionary<string, string> clanMember in FriendsController.sharedController.clanMembers)
		{
			...
			{
				foreach (KeyValuePair<string, object> item2 in value4 as Dictionary<string, object>)
				{
	...
	public void FillClanAttrs(Dictionary<string, string> plDict)
	{
		...
		if (plDict.ContainsKey("clan_creator_id") && plDict["clan_creator_id"] != null && id != null)
		{
			...
			avatarButton.GetComponent<UIButton>().normalSprite = (flag ? "avatar_leader_frame" : "avatar_frame");
		}
	}
	...
	private void UpdateOnline()
	{
		...
		if (FriendsController.sharedController.onlineInfo.ContainsKey(id))
		{
			...
			if ((float)result2 > FriendsController.onlineDelta || (num != 3 && ((num != (int)GameConnect.myPlatformConnect && num != -1) || GameConnect.GetTierForRoom() != result)))
			{
			...
			string value;
			if (FriendsController.sharedController.onlineInfo[id].TryGetValue("map", out value) && SceneInfoController.instance.GetInfoScene(int.Parse(value)) == null)
			{
				join.GetComponent<UIButton>().isEnabled = false;
			}
		}
	...
	private bool IsFriendInClan()
	{
		...
		return !string.IsNullOrEmpty(Convert.ToString(dictionary["clan_creator_id"]));
	}
