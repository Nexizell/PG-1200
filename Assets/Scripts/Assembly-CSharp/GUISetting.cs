...
public sealed class GUISetting : MonoBehaviour
{
	...
	private void OnGUI()
	{
		...
		@bool = GUI.Toggle(position, @bool, "", soundOnOff);
		...
		isChatOn = GUI.Toggle(position2, isChatOn, "", soundOnOff);
		Defs.IsChatOn = isChatOn;
		PlayerPrefs.Save();
		if (GUI.Button(new Rect(21f * num, (float)Screen.height - (21f + (float)back.normal.background.height) * num, (float)back.normal.background.width * num, (float)back.normal.background.height * num), "", back))
		{
			Singleton<SceneLoader>.Instance.LoadScene(Defs.MainMenuScene);
		}
		GUI.enabled = !StoreKitEventListener.purchaseInProcess;
		if (GUI.Button(new Rect((float)Screen.width / 2f - (float)restore.normal.background.width * num * 0.5f, (float)Screen.height - (21f + (float)restore.normal.background.height) * num, (float)restore.normal.background.width * num, (float)restore.normal.background.height * num), string.Empty, restore))
		{
		...
		mySens = GUI.HorizontalSlider(position3, Defs.Sensitivity, 6f, 18f, sliderStyle, thumbStyle);
