...
public class EditorShopItem : MonoBehaviour
{
	...
	public void SetData(EditorShopItemData data)
	{
		...
		stringBuilder.AppendLine(string.Format("Name: {0}", new object[1] { byDefault }));
		if (!string.IsNullOrEmpty(data.prefabName))
		{
			prefabName = data.prefabName;
			stringBuilder.AppendLine(string.Format("Prefab: {0}", new object[1] { data.prefabName }));
		}
		stringBuilder.Append(string.Format("Tag: {0}", new object[1] { data.tag }));
