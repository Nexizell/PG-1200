...
namespace RilisoftBot
{
	public class MeleeShootBot : ShootingBot
	{
		...
		private string GameNameMeleeAnimation()
		{
			...
			string text = modelCollider.gameObject.name;
			return string.Format("{0}_shooting", new object[1] { text });
		}
		...
		{
			if (Vector3.SqrMagnitude(target.transform.position - base.transform.position) < meleeAttackDistance * meleeAttackDistance)
			{
