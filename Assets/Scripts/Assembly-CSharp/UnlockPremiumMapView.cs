...
public sealed class UnlockPremiumMapView : MonoBehaviour
{
	...
	public int Price
	{
		...
		set
		{
			...
			if (priceSprite != null)
			{
				priceSprite.spriteName = string.Format("premium_baner_{0}", new object[1] { value });
			}
		}
	}
	...
	private void Start()
	{
		...
		if (priceSprite != null)
		{
			priceSprite.spriteName = string.Format("premium_baner_{0}", new object[1] { _price });
		}
		SetLabelPrice();
	}
	...
	private void SetLabelPrice()
	{
		if (priceLabel != null && priceLabel.Length != 0)
		{
			...
			{
				_price,
				LocalizationStore.Get("Key_1041")
			});
