...
public sealed class coinsPlashka : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		hidePlashka();
		tekKolCoins = Storager.getInt("Coins");
	...
	private void Update()
	{
		...
		if (tekKolCoins >= 500 && !isHasKeyAchived500)
		{
			...
			{
				Debug.Log(string.Format("Achievement Ekonomist completed: {0}", new object[1] { success }));
			});
		...
		if (tekKolCoins >= 1000 && !isHasKeyAchived1000)
		{
			...
			{
				Debug.Log(string.Format("Achievement Rich Man completed: {0}", new object[1] { success }));
			});
