		...
		try
		{
			...
			case EggHatchedType.Time:
				if (egg.IncubationTimeLeft.HasValue)
				{
					...
					return (value >= 86400) ? string.Format("{0} {1}", new object[2]
					{
						LocalizationStore.Get("Key_1125"),
						RiliExtensions.GetTimeStringDays(value)
					}) : RiliExtensions.GetTimeString(value);
				}
				return LocalizationStore.Get("Key_2566");
			case EggHatchedType.Wins:
				return string.Format(LocalizationStore.Get("Key_2511"), new object[1] { egg.WinsLeft });
			case EggHatchedType.Rating:
				return string.Format(LocalizationStore.Get("Key_2732"), new object[1] { egg.RatingLeft });
