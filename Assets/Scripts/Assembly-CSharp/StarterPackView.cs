...
public class StarterPackView : BannerWindow
{
	...
	private void CenterItems(int countHideElements)
	{
		if (items.Length >= 2 && countHideElements != 0)
		{
			...
			float num2 = (items[1].transform.localPosition.x - items[0].transform.localPosition.x) * num;
			int num3 = items.Length - countHideElements;
			for (int i = 0; i < num3; i++)
			{
				Vector3 localPosition = items[i].transform.localPosition;
				items[i].transform.localPosition = new Vector3(localPosition.x + num2, localPosition.y, localPosition.z);
			}
		}
	}
	...
	private void SetButtonText()
	{
		if (StarterPackController.Get.IsPackSellForGameMoney())
		{
			...
			if (priceDataForItemsPack != null)
			{
				...
				buttonMoneyIcon.spriteName = ((priceDataForItemsPack.Currency == "GemsCurrency") ? "gem_znachek" : "coin_znachek");
		...
		else
		{
			...
			buttonLabel.text = string.Format("{0} {1}", new object[2]
			{
				LocalizationStore.Get("Key_1043"),
				priceLabelForCurrentPack
			});
		}
	}
	...
	private void SetSaleLabel(int sale)
	{
		...
		{
			moneySaleLabel[i].text = string.Format("{0}% {1}", new object[2]
			{
				sale,
				LocalizationStore.Get("Key_0276")
			});
		}
	}
