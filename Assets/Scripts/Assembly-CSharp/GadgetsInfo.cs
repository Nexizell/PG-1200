...
public class GadgetsInfo
{
	...
	public static Dictionary<GadgetInfo.GadgetCategory, List<string>> GetGadgetsByCategoriesFromItems(List<string> items)
	{
		try
		{
			return (from gadgetId in items.Intersect(info.Keys)
				select info[gadgetId] into gadgetInfo
				group gadgetInfo by gadgetInfo.Category).ToDictionary((IGrouping<GadgetInfo.GadgetCategory, GadgetInfo> grouping) => grouping.Key, (IGrouping<GadgetInfo.GadgetCategory, GadgetInfo> grouping) => grouping.Select((GadgetInfo gadgetInfo) => gadgetInfo.Id).ToList());
		}
		catch (Exception ex)
		{
	...
	public static List<string> GetNewGadgetsForTier(int tier)
	{
		try
		{
			return (from chain in UpgradeChains
				select info[chain.First()] into gadgetInfo
	...
	public static void ProvideGadget(string gadgetId)
	{
		...
		do
		{
			if (Storager.getInt(list[index]) == 0)
			{
				Storager.setInt(list[index], 1);
	...
	{
		return Storager.getString(SNForCategory(category));
	}
	...
	public static string FirstUnboughtOrForOurTier(string gadgetId)
	{
		...
		if (list.IndexOf(text2) <= list.IndexOf(text))
		{
			return text;
		}
		return text2;
	...
	public static bool IsBought(string gadgetId)
	{
		...
		return Storager.getInt(gadgetId) > 0;
	}
	...
	private static void InitFirstForOurTierData()
	{
		...
		{
			Storager.setString("GadgetsInfo.FirstForOurTier", "{}");
		}
		string @string = Storager.getString("GadgetsInfo.FirstForOurTier");
		try
		{
	...
	{
		Storager.setString("GadgetsInfo.FirstForOurTier", Json.Serialize(firstGadgetsForOurTier));
	}
	...
	private static void InitializeUpgrades()
	{
		...
		_upgrades = (from kvp in info
			group kvp by BaseOfUpgardesChainFor(kvp.Value).Id into grouping
			select (from kvp in grouping.OrderBy(delegate(KeyValuePair<string, GadgetInfo> kvp)
				...
				select kvp.Key).ToList()).SelectMany((List<string> upgradesChain) => upgradesChain.Select((string upgrade) => new KeyValuePair<string, List<string>>(upgrade, upgradesChain))).ToDictionary((KeyValuePair<string, List<string>> kvp) => kvp.Key, (KeyValuePair<string, List<string>> kvp) => kvp.Value);
	...
	private static GadgetInfo BaseOfUpgardesChainFor(GadgetInfo gadgetInfo)
	{
		...
		if (gadgetInfo.PreviousUpgradeId == null)
		{
			return gadgetInfo;
		}
		return BaseOfUpgardesChainFor(info[gadgetInfo.PreviousUpgradeId]);
	}
	...
	static GadgetsInfo()
	{
		...
		_info = new Dictionary<string, GadgetInfo>
		{
			...
			{
				"gadget_firecrackers",
				new GadgetInfo(GadgetInfo.GadgetCategory.Tools, "gadget_firecrackers", "Key_2908", 1, null, "gadget_firecrackers_up1", "Key_2957", new List<GadgetInfo.Parameter>
				{
					GadgetInfo.Parameter.Damage,
					GadgetInfo.Parameter.Lifetime,
					GadgetInfo.Parameter.Cooldown
			...
			{
				"gadget_firecrackers_up1",
				new GadgetInfo(GadgetInfo.GadgetCategory.Tools, "gadget_firecrackers_up1", "Key_2955", 3, "gadget_firecrackers", "gadget_firecrackers_up2", "Key_2957", new List<GadgetInfo.Parameter>
				{
					GadgetInfo.Parameter.Damage,
					GadgetInfo.Parameter.Lifetime,
					GadgetInfo.Parameter.Cooldown
			...
			{
				"gadget_firecrackers_up2",
				new GadgetInfo(GadgetInfo.GadgetCategory.Tools, "gadget_firecrackers_up2", "Key_2956", 5, "gadget_firecrackers_up1", null, "Key_2957", new List<GadgetInfo.Parameter>
				{
					GadgetInfo.Parameter.Damage,
					GadgetInfo.Parameter.Lifetime,
					GadgetInfo.Parameter.Cooldown
