...
public class HintObject : MonoBehaviour
{
	...
	private void Update()
	{
		if (myHint.indicateTarget && lastIndic < Time.time)
		{
			...
			if (myHint.targetSprites == null || myHint.targetSprites.Length == 0)
			{
				myHint.targetSprite.spriteName = (indicOn ? myHint.indicatedSpriteName : myHint.defaultSpriteName);
			}
			else
			{
				for (int i = 0; i < myHint.targetSprites.Length; i++)
				{
					myHint.targetSprites[i].color = (indicOn ? Color.green : Color.white);
				}
			}
		}
