...
public class BtnBannerNewGun : ButtonBannerBase
{
	...
	public override void OnUpdateParameter()
	{
		...
		if (!string.IsNullOrEmpty(tagForClick))
		{
			...
			ItemDb.GetItemCategory(tagForClick);
			...
			{
				LocalizationStore.Key_0419,
				PromoActionsManager.sharedManager.discounts[tagForClick][0].Value
			});
			lbPrice.text = PromoActionsManager.sharedManager.discounts[tagForClick][1].Value.ToString();
			lbPrice.color = (priceByShopId.Currency.Equals("Coins") ? new Color(1f, 0.8627f, 0f) : new Color(0.3176f, 0.8117f, 1f));
			sprCoinImg.spriteName = (priceByShopId.Currency.Equals("Coins") ? "ingame_coin" : "gem_znachek");
		}
	}
