...
public class StarterPackModel
{
	...
	public static DateTime GetTimeDataEvent(string timeEventKey)
	{
		...
		DateTime.TryParse(Storager.getString(timeEventKey), out result);
		return result;
	}

	public static string GetUrlForDownloadEventData()
	{
		string text = "https://secure.pixelgunserver.com/pixelgun3d-config/starterPack/";
		...
		return string.Format("{0}{1}", new object[2] { text, empty });
	}
