...
public sealed class FreeAwardView : MonoBehaviour
{
	...
	private void RefreshAwardLabel(bool visible)
	{
		...
		int num = FreeAwardController.CountMoneyForAward(serverTime.HasValue ? FreeAwardController.Instance.GetAdvertCountDuringCurrentPeriod(serverTime.Value) : ChestInLobbyPointMemento.DefaultAward);
		text += ((FreeAwardController.Instance.CurrencyForAward == "GemsCurrency") ? string.Format(CultureInfo.InvariantCulture, " [c][50CEFFFF]{0}[-][/c]", num) : num.ToString(CultureInfo.InvariantCulture));
	...
	private void Update()
	{
		...
		if (waitingState != null)
		{
			...
			if (loadingSpinner != null)
			{
				...
				loadingSpinner.fillAmount = (loadingSpinner.invert ? (num - (float)num2) : (1f - num + (float)num2));
			}
		}
	...
	{
		CurrencySpecificWatchPanel currencySpecificWatchPanel = ((FreeAwardController.Instance.CurrencyForAward == "GemsCurrency") ? watchForGemsPanel : watchForCoinsPanel);
		...
		string text = ((nextTimeAwailable.Hours > 0) ? string.Format("{0}:{1:D2}:{2:D2}", new object[3] { nextTimeAwailable.Hours, nextTimeAwailable.Minutes, nextTimeAwailable.Seconds }) : string.Format("{0}:{1:D2}", new object[2] { nextTimeAwailable.Minutes, nextTimeAwailable.Seconds }));
	...
	{
		List<UILabel> result = new List<UILabel>(3) { rootLabel };
