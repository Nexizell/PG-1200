...
public class PreviewSkin : MonoBehaviour
{
	...
	private void Update()
	{
		...
		{
			touchPosition = ((Input.touchCount > 0) ? Input.GetTouch(0).position : new Vector2(Input.mousePosition.x, Input.mousePosition.y));
		...
		{
			float num = ((Input.touchCount > 0) ? (touchPosition.x - Input.GetTouch(0).position.x) : (touchPosition.x - Input.mousePosition.x));
