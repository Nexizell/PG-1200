...
public class DuelController : MonoBehaviour
{
	...
	public RoomStatus roomStatus
	{
		get
		{
			return (RoomStatus)PhotonNetwork.room.customProperties[GameConnect.roomStatusProperty];
		}
		set
		{
	...
	public bool showEnemyCharacter
	{
		get
		{
			if ((opponentNetworkTable != null || opponentLeftInEnd) && duelUI != null && duelUI.showCharacters && gameStatus != GameStatus.WaitForOpponent && gameStatus != GameStatus.OpponentConnected)
			{
				return gameStatus != GameStatus.ChangeArea;
			}
			return false;
		}
	}
	...
	public DuelUIController duelUI
	{
		get
		{
			...
			return NetworkStartTableNGUIController.sharedController.duelUI;
		}
	}
	...
	private void Awake()
	{
		base.enabled = GameConnect.isDuel && Defs.isMulti;
		instance = this;
	...
	public void SetEnemySkin(byte[] skin)
	{
		...
		texture2D.Apply();
		enemyCharacter.SetSkin(texture2D, (enemyCharacter.weapon != null) ? enemyCharacter.weapon.GetComponent<WeaponSounds>() : null);
	}
	...
	{
		enemyCharacter.SetWeapon(weapon, altWeapon, "");
	}
	...
	{
		string @string = Storager.getString(Defs.ArmorNewEquppedSN);
	...
	{
		string @string = Storager.getString("MaskEquippedSN");
		photonView.RPC("SetEnemyMask", PhotonTargets.Others, @string);
	}

	public void SendMyBoots()
	{
		string @string = Storager.getString(Defs.BootsEquppedSN);
	...
	{
		myCharacter.SetSkin(SkinsController.currentSkinForPers, (myCharacter.weapon != null) ? myCharacter.weapon.GetComponent<WeaponSounds>() : null);
	}

	public void SetMyCape()
	{
		string @string = Storager.getString(Defs.CapeEquppedSN);
		myCharacter.UpdateCape(@string, @string.Equals("cape_Custom") ? SkinsController.capeUserTexture : null, !ShopNGUIController.ShowWear);
	}
	...
	public void SetMyWeapon()
	{
		...
		Weapon weapon = WeaponManager.sharedManager.playerWeapons[index] as Weapon;
		WeaponSkin skinForWeapon = WeaponSkinsManager.GetSkinForWeapon(weapon.weaponPrefab.name);
		myCharacter.SetWeapon(weapon.weaponPrefab.name, weapon.weaponPrefab.name, (skinForWeapon != null) ? skinForWeapon.Id : string.Empty);
	}
	...
	private void ChangeRoomStatus(RoomStatus newStatus)
	{
		...
		Debug.Log((newStatus == RoomStatus.Closed) ? "Room closed!" : "Room opened!");
	}
	...
	private void Update()
	{
		...
		if (gameStatus == GameStatus.ReadyToStart)
		{
			...
			if (duelUI != null)
			{
				...
				duelUI.versusTimer.text = ((Mathf.Floor(goTimer) <= 0f) ? "GO!" : num.ToString());
			}
			if (goTimer < 0f)
			{
				goTimer = 0f;
	...
	{
		GameObject obj = Resources.Load<GameObject>("Character_model");
		obj.SetActive(false);
		GameObject obj2 = UnityEngine.Object.Instantiate(obj, transform.position, transform.rotation);
		CharacterInterface component = obj2.GetComponent<CharacterInterface>();
	...
	public void SetShopEvents()
	{
		...
		sharedShop.onEquipSkinAction = (Action<string>)Delegate.Combine(sharedShop.onEquipSkinAction, new Action<string>(SkinEquip));
	...
	private void SkinEquip(string id)
	{
		SetMySkin();
		SendMySkin();
	}

	private void RemoveShopEvents()
	{
		ShopNGUIController sharedShop = ShopNGUIController.sharedShop;
		sharedShop.onEquipSkinAction = (Action<string>)Delegate.Remove(sharedShop.onEquipSkinAction, new Action<string>(SkinEquip));
		ShopNGUIController.ShowWearChanged -= OnMyWearVisibleChanged;
		ShopNGUIController.ShowArmorChanged -= OnMyArmorVisibleChanged;
		if (equippedPetActionSet)
		{
			equippedPetActionSet = false;
			ShopNGUIController.EquippedPet -= OnPetEquipAction;
			ShopNGUIController.UnequippedPet -= OnPetUnequipAction;
		}
	}
