...
public class ChestBonusItem : MonoBehaviour
{
	...
	public void SetData(ChestBonusItemData itemData)
	{
		...
		empty = ((itemData.timeLife != -1) ? itemData.GetTimeLabel() : ((itemData.count <= 1) ? LocalizationStore.Get("Key_1059") : string.Format("{0} {1}", new object[2]
		{
			itemData.count,
			LocalizationStore.Get("Key_1230")
		})));
