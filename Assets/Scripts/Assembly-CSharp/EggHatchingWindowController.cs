...
public class EggHatchingWindowController : GeneralBannerWindow
{
	...
	[CompilerGenerated]
	internal sealed class _003CReturnEggToIsPosition_003Ed__73 : IEnumerator<object>, IEnumerator, IDisposable
	{
		private int _003C_003E1__state;

		private object _003C_003E2__current;

		public EggHatchingWindowController _003C_003E4__this;
		...
		[DebuggerHidden]
		public _003CReturnEggToIsPosition_003Ed__73(int _003C_003E1__state)
		{
		...
		private bool MoveNext()
		{
			...
			case 1:
				_003C_003E1__state = -1;
				_003C_003E4__this.egg.localPosition -= _003C_003E4__this.EGG_HIDE_OFFSET;
				return false;
			}
		}
	...
	public Egg EggForHatching
	{
		...
		set
		{
			...
			if (_3dModelsParent != null && m_eggForHatching != null)
			{
				...
				{
					UnityEngine.Debug.LogErrorFormat("Exception in setting egg's texture: {0}", ex);
				}
			}
		}
	}
	...
	protected bool IsPetOfMaxUpgrade
	{
		get
		{
			...
			{
				UnityEngine.Debug.LogErrorFormat("Exception in IsPetOfMaxUpgrade: {0}", ex);
			}
			return false;
		}
	}
	...
	public Transform ReplaceEggWithPet(string petId)
	{
		...
		Transform component = UnityEngine.Object.Instantiate(Resources.Load<PetEngine>(Singleton<PetsManager>.Instance.GetRelativePrefabPath(petId)).Model).GetComponent<Transform>();
	...
	public virtual void HandleHatchButton()
	{
		...
		{
			UnityEngine.Debug.LogErrorFormat("Exception in getting eggOpenDuraition: {0}", ex);
		}
		try
		{
			...
			HOTween.To(p_parms: new TweenParms().Prop("color", new PlugSetColor(new Color(1f, 1f, 1f, 0f)).Property("_Color")).Ease(EaseType.Linear).UpdateType(UpdateType.TimeScaleIndependentUpdate)
				.Delay(p_delay), p_target: eggRenderer.material, p_duration: num);
		}
		catch (Exception ex2)
		{
			UnityEngine.Debug.LogErrorFormat("Exception in starting fading out egg: {0}", ex2);
		}
		...
		{
			UnityEngine.Debug.LogErrorFormat("Exception in calculating shouldEquipPet: {0}", ex3);
		}
		SetPetId(infoId);
		PetInfo info = petUpdateInfo.PetNew.Info;
		Transform obj = ReplaceEggWithPet(info.Id);
		obj.localScale = Vector3.zero;
		HOTween.To(p_parms: new TweenParms().Prop("localScale", Vector3.one).Ease(EaseType.Linear).UpdateType(UpdateType.TimeScaleIndependentUpdate)
			.Delay(p_delay)
			.OnComplete((TweenDelegate.TweenCallback)delegate
			{
			}), p_target: obj, p_duration: num / 2f);
		try
		{
			...
			else
			{
				...
				{
					item.text = string.Format(LocalizationStore.Get("Key_2596"), new object[1] { LocalizationStore.Get(info.Lkey) });
				}
				...
				if (!isPetOfMaxUpgrade)
				{
					...
					{
						item3.text = string.Format(LocalizationStore.Get("Key_2496"), new object[1] { info.Up + 1 });
					}
					foreach (UILabel item4 in nextGrade)
					{
						item4.text = string.Format(LocalizationStore.Get("Key_2496"), new object[1] { info.Up + 2 });
					}
				}
				else
				{
					RiliExtensions.ForEach(currentGrade, delegate(UILabel lab)
					{
						lab.gameObject.SetActiveSafeSelf(false);
					});
					RiliExtensions.ForEach(nextGrade, delegate(UILabel lab)
					{
						lab.gameObject.SetActiveSafeSelf(false);
					});
				}
				...
				if (!isPetOfMaxUpgrade)
				{
					...
					{
						point.text = string.Format("{0} {1} / {2}", new object[3]
						{
				...
				{
					RiliExtensions.ForEach(points, delegate(UILabel lab)
					{
						lab.gameObject.SetActiveSafeSelf(false);
					});
				}
				...
				RiliExtensions.ForEach(maximumLevel, delegate(UILabel lab)
				{
		...
		{
			UnityEngine.Debug.LogErrorFormat("Exception in HandleHatchButton in hatching banner: {0}", ex4);
		}
		...
		HOTween.To((petUpdateInfo.PetAdded ? newPetState : upgradePetState).GetComponent<UIPanel>(), 0.4f, p_parms3);
	}
	...
	public void SavePetsName()
	{
		try
		{
			if (m_petId.IsNullOrEmpty())
			{
				UnityEngine.Debug.LogErrorFormat("SavePetsName m_petId.IsNullOrEmpty()");
				return;
			}
		...
		{
			UnityEngine.Debug.LogErrorFormat("Exception in setting pets name after hatching: {0}", ex);
		}
	}
	...
	{
		UnityEngine.Debug.Log(">>>> OnBuyPetFailure");
	}
	...
	private void PlayPetAnimation(GameObject objectWithAnimation)
	{
		...
		try
		{
			if (!ShopNGUIController.IsModeWithNormalTimeScaleInShop())
			{
				...
				{
					UnityEngine.Debug.LogErrorFormat("Error: pet {0} has no Profile animation clip (EggHatchingWindowController)", objectWithAnimation.nameNoClone());
				}
	...
	private int CoinsForPetMaxUpggrade(string petId)
	{
		...
		{
			UnityEngine.Debug.LogErrorFormat("Exception in CoinsForPetMaxUpggrade: {0}", ex);
		}
		return 5;
	}
	...
	private void EquipPetIfNeeded()
	{
		if (m_petId == null)
		{
			UnityEngine.Debug.LogErrorFormat("EquipPetIfNeeded: m_petId == null");
		...
		if (ShouldEquipPet)
		{
			...
			{
				UnityEngine.Debug.LogErrorFormat("Exception in equipping pet in EggHatchingWindowController: {0}", ex);
			}
		}
		ShouldEquipPet = false;
	}
