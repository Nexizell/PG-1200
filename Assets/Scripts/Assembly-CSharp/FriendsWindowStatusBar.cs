...
public class FriendsWindowStatusBar : MonoBehaviour
{
	...
	{
		string text = string.Format(LocalizationStore.Get("Key_1416"), new object[2]
		{
			Defs.maxCountFriend,
			Defs.maxCountFriend
		});
	...
	private void SetTextFacebookButton(bool isLogin, bool isRewardTake)
	{
		...
		if (isRewardTake)
		{
			...
			string facebookNotRewardButtonText = ((!isLogin) ? LocalizationStore.Get("Key_1582") : LocalizationStore.Get("Key_1437"));
		...
		{
			facebookRewardLabels[i].text = string.Format("+{0}", new object[1] { 10 });
		}
	}
	...
	private void SetupStateFacebookContainer(bool needHide)
	{
		...
		bool isRewardTake = Storager.getInt(Defs.IsFacebookLoginRewardaGained) == 1;
