	...
	public class PetIndicationController : MonoBehaviour
	{
		...
		public void CreateFrameLabel()
		{
			...
			for (int j = 0; j < labelsFrame.Count; j++)
			{
				...
				labelsFrame[j].GetComponent<TextMesh>().offsetZ = 0.2f;
			}
		}
		...
		private void SetLabelColor()
		{
			...
			if (Defs.isMulti)
			{
				...
				else if (GameConnect.isDaterRegim || GameConnect.gameMode == GameConnect.GameMode.TimeBattle)
				{
					TextMesh.color = ((_engine.Owner == WeaponManager.sharedManager.myPlayerMoveC) ? ColorPetMy : ColorPetCoop);
				}
				else
				{
					TextMesh.color = (_engine.IsMine ? ColorPetMy : ColorPetEnemy);
				}
			}
			else
			{
