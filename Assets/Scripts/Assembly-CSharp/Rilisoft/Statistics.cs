...
namespace Rilisoft
{
	internal sealed class Statistics
	{
		...
		private Dictionary<string, int> LoadPopularityFromPlayerPrefs(string playerPrefsKey)
		{
			...
			Dictionary<string, object> dictionary2 = Json.Deserialize(PlayerPrefs.GetString(playerPrefsKey, "{}")) as Dictionary<string, object>;
			if (dictionary2 == null)
			{
				return dictionary;
			}
		...
		private List<Dictionary<string, int>> LoadPopularityForTierFromPlayerPrefs(string playerPrefsKey)
		{
			...
			List<object> list2 = Json.Deserialize(PlayerPrefs.GetString(playerPrefsKey, "{}")) as List<object>;
			if (list2 == null)
			{
				return list;
			}
			for (int j = 0; j < ExpController.LevelsForTiers.Length; j++)
			{
				...
				foreach (KeyValuePair<string, object> item in list2[j] as Dictionary<string, object>)
				{
		...
		private void SavePopularityInfo(string playerPrefsKey, object popularityInfo)
		{
			...
			{
				Debug.Log(string.Format("Saving: playerPrefsKey: {0}, popularityInfo: {1}", new object[2] { playerPrefsKey, text }));
			}
