...
namespace Rilisoft
{
	public sealed class BackSystem : MonoBehaviour
	{
		...
		public IDisposable Register(Action callback, string context = null)
		{
			...
			{
				Debug.Log(string.Format("<color=lightblue>Back stack after registration: {0}</color>", new object[1] { this }));
			}
			return result;
		}
		...
		private void Update()
		{
			if (EscapePressed())
			{
				...
				string text = (Application.isEditor ? ToString() : string.Empty);
				CollectGarbage();
				...
				if (last != null)
				{
					...
					if (Application.isEditor)
					{
						Debug.Log(string.Format("<color=#db7093ff>Back stack on invoke: {0} -> {1}</color>", new object[2] { text, this }));
					}
				}
			}
			else if (Input.GetKeyUp(KeyCode.Backspace) && Application.isEditor)
			{
				Debug.Log(string.Format("<color=#db7093ff>Current back stack: {0}</color>", new object[1] { this }));
			}
		}
		...
		private static BackSystem InitializeInstance()
		{
			...
			GameObject obj = new GameObject("Rilisoft.BackSystem");
			UnityEngine.Object.DontDestroyOnLoad(obj);
			return obj.AddComponent<BackSystem>();
		}
	}
}
