...
namespace Rilisoft
{
	internal static class QuestConstants
	{
		...
		internal static ShopNGUIController.CategoryNames? ParseWeaponSlot(string weaponSlot)
		{
			...
			try
			{
				return (ShopNGUIController.CategoryNames)Enum.Parse(typeof(ShopNGUIController.CategoryNames), weaponSlot);
			}
			catch
			{
				return null;
		...
		internal static GameConnect.GameMode? ParseMode(string mode)
		{
			...
			try
			{
				return (GameConnect.GameMode)Enum.Parse(typeof(GameConnect.GameMode), mode);
			}
			catch
			{
				return null;
		...
		public static string GetAccumulativeQuestDescriptionByType(AccumulativeQuestBase quest)
		{
			...
			{
				string value2;
				if (!GameConnect.gameModesLocalizeKey.TryGetValue(Convert.ToInt32(modeAccumulativeQuest.Mode), out value2))
				{
					value2 = modeAccumulativeQuest.Mode.ToString();
					Debug.LogError("Couldnot find mode name for " + modeAccumulativeQuest.Mode);
				}
				return string.Format(text, new object[2]
				{
					string.Format("[fff600]{0}[-]", new object[1] { modeAccumulativeQuest.RequiredCount }),
					string.Format("[ff9600]{0}[-]", new object[1] { LocalizationStore.Get(value2) })
				});
			}
			...
			if (mapAccumulativeQuest != null)
			{
				...
				return string.Format(text, new object[2]
				{
					string.Format("[fff600]{0}[-]", new object[1] { mapAccumulativeQuest.RequiredCount }),
					string.Format("[ff9600]{0}[-]", new object[1] { empty })
				});
			}
			...
			if (weaponSlotAccumulativeQuest != null)
			{
				...
				return string.Format(text, new object[2]
				{
					string.Format("[fff600]{0}[-]", new object[1] { weaponSlotAccumulativeQuest.RequiredCount }),
					string.Format("[ff9600]{0}[-]", new object[1] { LocalizationStore.Get(value3) })
				});
			}
			try
			{
				return string.Format(text, new object[1] { string.Format("[fff600]{0}[-]", new object[1] { quest.RequiredCount }) });
			}
