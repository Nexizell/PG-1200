...
namespace Rilisoft
{
	public class GroundPetEngine : PetEngine
	{
				...
				else if (ctx.Nma.enabled && !ctx.InRange(ctx.MovePosition, ctx.ThisTransform.position, ctx.Info.MinToOwnerDistance))
				{
					if (ctx.CanMoveToPlayer || !ctx.OwnerIsGrounded)
					{
						if (!ctx.Nma.isOnOffMeshLink)
						{
							...
							ctx.Nma.speed = ctx.CalculateSpeedMultyplierByDistance() * ctx.Info.SpeedModif * num;
							ctx.Nma.Resume();
						}
					}
					else
		...
		internal class PetMoveToTargetState : State<PetState>
		{
			...
			public override void Update()
			{
				...
				else if (!ctx.Nma.isOnOffMeshLink)
				{
					...
					float num = EffectsController.PetSpeedModificator();
					float max = ctx.Info.SpeedModif * num;
					ctx.Nma.speed = Mathf.Clamp(ctx.CalculateSpeedMultyplierByDistance() * ctx.Info.SpeedModif * num, 0f, max);
					ctx.Nma.Resume();
				}
			}
		...
		public override Vector3 MovePosition
		{
			get
			{
				if (!(base.Owner.GetPointForGroundPet() != null))
				{
					return base.Owner.transform.position;
				}
				return base.Owner.GetPointForGroundPet().position;
			}
		}
		...
		public bool CanTeleportToTarget
		{
			get
			{
				...
				if ((Target.position - ThisTransform.position).magnitude > base.Info.ToTargetTeleportDistance)
				{
					return false;
				}
