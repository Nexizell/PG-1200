...
namespace Rilisoft
{
	public class DirectionViewer : MonoBehaviour
	{
		...
		[SerializeField]
		protected internal List<DirectionViewerSettings> _settings = new List<DirectionViewerSettings>
		{
		...
		private void Awake()
		{
			...
			_freePointers.Clear();
			foreach (DirectionPointer item in GetComponentsInChildren<DirectionPointer>(true).ToList())
			{
		...
		{
			RiliExtensions.ForEach(_activePointers.ToList(), delegate(DirectionPointer p)
			{
				ForgetPointer(p);
			});
		}
		...
		private float GetAngle(Transform from, Vector3 target, Vector3 n)
		{
			...
			return Mathf.Atan2(Vector3.Dot(n, Vector3.Cross(forward, rhs)), Vector3.Dot(forward, rhs)) * (180f / (float)Math.PI);
		}
