...
namespace Rilisoft
{
	public class WeaponSkinsManager
	{
		...
		public static List<string> SkinIds;
		...
		public static bool SetSkinToCurrentWeapon(string skinId)
		{
			...
			ItemRecord byPrefabName = ItemDb.GetByPrefabName(WeaponManager.sharedManager.currentWeaponSounds.nameNoClone());
			if (byPrefabName == null)
			{
				return false;
			}
			...
			bool num = skin.SetTo(WeaponManager.sharedManager.currentWeaponSounds.gameObject);
			if (num)
			{
				Storager.setString(StoragerNameForEquippedSkinForWeapon(itemRecord.Tag), skin.Id);
				UpdateWeaponSkins(byPrefabName.Tag);
			}
			return num;
		}
		...
		public static bool SetSkinToWeapon(string skinId, string weaponName)
		{
			...
			Storager.setString(StoragerNameForEquippedSkinForWeapon(text), skinId);
			Action<ItemRecord, string> equippedSkinForWeapon = WeaponSkinsManager.EquippedSkinForWeapon;
			if (equippedSkinForWeapon != null)
			{
		...
		public static string GetSettedSkinId(string weaponName)
		{
			...
			return GetSettedSkinIdByWeaponTag(byPrefabName.Tag);
		}
		...
		public static string GetSettedSkinIdByWeaponTag(string weaponTag)
		{
			...
			return Storager.getString(StoragerNameForEquippedSkinForWeapon(weaponTag));
		}
		...
		{
			return Storager.getInt(skinId) > 0;
		}
		...
		{
			if (skin != null && !IsBoughtSkin(skin.Id))
			{
				return skin.ForLeague <= RatingSystem.instance.currentLeague;
			}
			return false;
		}
