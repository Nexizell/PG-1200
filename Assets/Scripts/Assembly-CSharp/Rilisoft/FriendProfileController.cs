...
namespace Rilisoft
{
	internal sealed class FriendProfileController : IDisposable
	{
		...
		{
			bool num = FriendsController.IsPlayerOurClanMember(playerId);
			bool flag = FriendsController.IsSelfClanLeader();
			...
			bool activeChatButton = _isPlayerOurFriend && type == ProfileWindowType.friend && !flag2;
			bool flag7 = !num && flag && !flag2 && !flag6;
			bool flag8 = !_isPlayerOurFriend && !flag2 && !flag5;
			bool activeRemoveButton = _isPlayerOurFriend && !flag2;
			_friendProfileView.SetActiveAddButton(flag8 && !flag3);
			_friendProfileView.SetActiveAddButtonSent(flag8 && flag3);
			_friendProfileView.SetActiveInviteButton(flag7 && !flag4);
			_friendProfileView.SetActiveAddClanButtonSent(flag7 && flag4);
		...
		private void Initialize(IFriendsGUIController friendsGuiController, bool oldInterface = true)
		{
			...
			string path = "FriendProfileView(UI)";
		...
		private void UpdateAllData(string friendId)
		{
			...
			try
			{
				Dictionary<string, object> dictionary = fullPlayerDataById["player"] as Dictionary<string, object>;
				if (dictionary != null && dictionary.ContainsKey("lobby_likes"))
				{
					int num = Convert.ToInt32(dictionary["lobby_likes"]);
					if (num != _friendProfileView.LikesNumFromServer)
					{
						_friendProfileView.LikesNumAdding = 0;
					}
					_friendProfileView.LikesNumFromServer = num;
				}
			}
			catch (Exception ex)
			{
				Debug.LogWarningFormat("Exception in getting num of likes: {0}", ex);
			}
			try
			{
				Dictionary<string, object> dictionary2 = fullPlayerDataById["player"] as Dictionary<string, object>;
				if (dictionary2 != null && dictionary2.ContainsKey("lobby"))
				{
					object obj = dictionary2["lobby"];
					if (obj != null)
					{
						_friendProfileView.SetLobby((obj as List<object>).OfType<string>().ToList());
					}
				}
			}
			catch (Exception ex2)
			{
				Debug.LogWarningFormat("Exception in setting lobby for friend: {0}", ex2);
			}
		}
			...
			if (dictionary.TryGetValue("clan_name", out value5))
			{
				...
				if (!string.IsNullOrEmpty(text2))
				{
					...
					if (text2 != null && text2.Length > num)
					{
						...
						{
							text2.Substring(0, (num - 2) / 2),
							text2.Substring(text2.Length - (num - 2) / 2, (num - 2) / 2)
						});
					}
		...
		internal void HandleProfileClickedCore(string id, ProfileWindowType type, Action<bool> onCloseEvent)
		{
			if (!_disposed)
			{
				...
				_friendProfileView.UpdateSeeLobbyButton();
			}
		}
		...
		private void HandleJoinClicked()
		{
			...
			else if (FriendsController.sharedController.onlineInfo.ContainsKey(_friendId))
			{
				...
				if (SceneInfoController.instance.GetInfoScene(int.Parse(text)) != null)
				{
