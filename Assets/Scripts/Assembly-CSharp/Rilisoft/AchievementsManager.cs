...
namespace Rilisoft
{
	public class AchievementsManager : Singleton<AchievementsManager>
	{
		...
		{
			return AchievementProgressData.ParseAll(Storager.getString("achievementsProgress"));
		}
		...
		protected internal override void Awake()
		{
			...
			foreach (AchievementData data in enumerable)
			{
				...
				bool isDebugBuild3 = Debug.isDebugBuild;
				Achievement ach = CreateAchievement(data, achievementProgressData);
				bool isDebugBuild4 = Debug.isDebugBuild;
				if (ach == null)
				{
					continue;
				}
		...
		private void OnAchievementProgressChanged(Achievement ach, bool pointsChanged, bool stageChanged)
		{
			...
			else if (stageChanged)
			{
				...
				InfoWindowController.ShowAchievementsBox(text, bgTexture, ach.Data.Icon, false);
			...
			{
				int num = GetAchievementsToShow().ToList().IndexOf(ach);
				if (num > -1)
				{
		...
		{
			return from a in _achievementsAll
		...
		{
			string val = Json.Serialize(_achievementsAll.Select((Achievement a) => a.ProgressData.ObjectForSave()).ToList());
			Storager.setString("achievementsProgress", val);
		}
	}
}
