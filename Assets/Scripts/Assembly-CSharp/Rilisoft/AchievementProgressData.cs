...
namespace Rilisoft
{
	...
	public class AchievementProgressData 
	{
		...
		public static AchievementProgressData Create(string raw)
		{
			...
			{
				return Create(Json.Deserialize(raw) as Dictionary<string, object>);
			}
			return null;
		}
		...
		public static AchievementProgressData Create(Dictionary<string, object> obj)
		{
			...
			{
				AchievementId = obj.ParseJSONField("id", ConvertToInt32Invariant),
				Points = obj.ParseJSONField("p", ConvertToInt32Invariant),
				Stage = obj.ParseJSONField("s", ConvertToInt32Invariant),
				CustomData = (obj.ParseJSONField("cd", (object o) => o as Dictionary<string, object>, true) ?? new Dictionary<string, object>())
			};
		}
		...
		public static string Merge(string rawListOne, string rawListTwo)
		{
			...
			return Json.Serialize(Merge(two: new AchievementProgressSyncObject(ParseAll(rawListTwo)), one: new AchievementProgressSyncObject(progressData)).ProgressData.Select((AchievementProgressData o) => o.ObjectForSave()).ToList());
		}
		...
		public static AchievementProgressSyncObject Merge(AchievementProgressSyncObject one, AchievementProgressSyncObject two)
		{
			...
			foreach (AchievementProgressData item in one.ProgressData)
			{
		...
		public static AchievementProgressData Merge(AchievementProgressData one, AchievementProgressData two)
		{
			...
			achievementProgressData = ((one.Stage != two.Stage) ? ((one.Stage > two.Stage) ? one : two) : ((one.Points == two.Points) ? one : ((one.Points > two.Points) ? one : two)));
			if (one.CustomData == null || two.CustomData == null)
			{
				achievementProgressData.CustomData = ((one.CustomData != null) ? one.CustomData : two.CustomData);
			}
			else
			{
				...
				if (all.Any())
				{
					foreach (KeyValuePair<string, object> item in all)
					{
						...
						{
							dictionary.Add(key, ((double)value > (double)value2) ? value : value2);
						}
						else if (value is long)
						{
							dictionary.Add(key, ((long)value > (long)value2) ? value : value2);
						}
						else if (value is int)
						{
							dictionary.Add(key, ((int)value > (int)value2) ? value : value2);
						}
						else if (value is string)
						{
							dictionary.Add(key, (value.ToString().Length > value2.ToString().Length) ? value : value2);
						}
