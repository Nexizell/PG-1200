...
namespace Rilisoft
{
	...
	internal struct CampaignProgressMemento : IEquatable<CampaignProgressMemento>
	{
		...
		{
			if (!EqualityComparer<List<LevelProgressMemento>>.Default.Equals(Levels, other.Levels))
			{
				return false;
			}
			return true;
		...
		internal static CampaignProgressMemento Merge(CampaignProgressMemento left, CampaignProgressMemento right)
		{
			...
			foreach (LevelProgressMemento item in from l in left.Levels.Concat(right.Levels)
				where l != null
				select l)
			{
				LevelProgressMemento value;
