...
namespace Rilisoft
{
	internal struct QuestImage
	{
		...
		private Dictionary<GameConnect.GameMode, string> _mapModeToSpriteName;
		...
		private Dictionary<GameConnect.GameMode, string> MapModeToSpriteName
		{
			get
			{
				if (_mapModeToSpriteName == null)
				{
					_mapModeToSpriteName = new Dictionary<GameConnect.GameMode, string>(new GameRegimeComparer())
					{
						{
		...
		private string GetSpriteNameForMultiplayer(GameConnect.GameMode mode)
		{
			string value;
