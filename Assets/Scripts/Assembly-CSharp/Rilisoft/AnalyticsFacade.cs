	...
	internal sealed class AnalyticsFacade
	{
		...
		private static string GetFlurryApiKey(HiddenSettings settings)
		{
			...
			if (BuildSettings.BuildTargetPlatform == RuntimePlatform.IPhonePlayer)
			{
				if (!flag)
				{
					return settings.flurryIosApiKey;
				}
				return settings.flurryIosDevApiKey;
			}
			if (BuildSettings.BuildTargetPlatform == RuntimePlatform.Android)
			{
				...
				case Defs.RuntimeAndroidEdition.Amazon:
					if (!flag)
					{
						return settings.flurryAmazonApiKey;
					}
					return settings.flurryAmazonDevApiKey;
				case Defs.RuntimeAndroidEdition.GoogleLite:
					if (!flag)
					{
						return settings.flurryAndroidApiKey;
					}
					return settings.flurryAndroidDevApiKey;
				}
			}
			return string.Empty;
		}
		...
		public static void LevelUp(int level, bool duplicateToConsole)
		{
			...
			Dictionary<string, int> dictionary = new Dictionary<string, int>
			{
				{
					"Coins",
					Storager.getInt("Coins")
				},
				{
					"GemsCurrency",
					Storager.getInt("GemsCurrency")
				}
			};
			if (_devToDevFacade != null)
			{
		...
		private static string InitializeParametrizedEventFormat()
		{
			...
			return "<color=magenta>\"{0}\": {1}</color>";
		}
	}
}
