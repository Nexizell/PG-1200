...
namespace Rilisoft
{
	...
	internal sealed class AdsConfigMemento 
	{
		...
		private readonly Dictionary<string, PlayerStateMemento> _playerStates;
		...
		internal static AdsConfigMemento FromJson(string json)
		{
			...
			if (obj == null)
			{
				return new AdsConfigMemento(new ArgumentException(string.Format(CultureInfo.InvariantCulture, "Failed to deserialize json: `{0}`", json), "json"));
			}
			Dictionary<string, object> dictionary = obj as Dictionary<string, object>;
			if (dictionary == null)
			{
				return new AdsConfigMemento(new ArgumentException(string.Format(CultureInfo.InvariantCulture, "Failed to interpret json as dictionary: `{0}`", json), "json"));
			}
			try
			{
		...
		internal static AdsConfigMemento FromDictionary(Dictionary<string, object> dictionary)
		{
			...
			AdsConfigMemento adsConfigMemento = new AdsConfigMemento((Exception)null);
			object value;
		...
		{
			_playerStates = new Dictionary<string, PlayerStateMemento>();
		...
		{
			return new InvalidOperationException(string.Format(CultureInfo.InvariantCulture, "Failed to interpret node as dictionary: `{0}`", key));
		}
	}
}
