...
{
	[Serializable]
	public sealed class FakeInterstitialConfigMemento 
	{
		...
		internal static FakeInterstitialConfigMemento FromDictionary(Dictionary<string, object> dictionary)
		{
			...
			fakeInterstitialConfigMemento.Enabled = ParsingHelper.GetBoolean(dictionary, "enabled") ?? false;
			...
			fakeInterstitialConfigMemento.MinLevel = ParsingHelper.GetInt32(dictionary, "minLevel") ?? 1;
			fakeInterstitialConfigMemento.MaxLevel = ParsingHelper.GetInt32(dictionary, "maxLevel") ?? int.MaxValue;
			fakeInterstitialConfigMemento.ShowFrequency = ParsingHelper.GetInt32(dictionary, "showFrequency") ?? 0;
			fakeInterstitialConfigMemento.MaxShowCountDuringSession = ParsingHelper.GetInt32(dictionary, "maxShowCountDuringSession") ?? 0;
		...
		internal string GetDisabledReason(string category, int level, int fakeInterstitialCount, int totalInterstitialCount, bool realInterstitialsEnabled)
		{
			...
			if (boolean.HasValue)
			{
				if (!boolean.Value)
				{
					return string.Format("Explicitely disabled for category `{0}`.", new object[1] { category });
				}
			}
			else if (!Enabled)
			{
			...
			if (@int.HasValue)
			{
				if (level < @int.Value)
				{
					return string.Format("Level {0} < {1} for category `{2}`.", new object[3] { level, @int.Value, category });
				}
			}
			else if (level < MinLevel)
			{
				return string.Format("Level {0} < {1}.", new object[2] { level, @int.Value });
			}
			...
			if (int2.HasValue)
			{
				if (level > int2.Value)
				{
					return string.Format("Level {0} > {1} for category `{2}`.", new object[3] { level, int2.Value, category });
				}
			}
			else if (level > MaxLevel)
			{
				return string.Format("Level {0} > {1}.", new object[2] { level, int2.Value });
			}
			...
			if (realInterstitialsEnabled)
			{
				...
				else
				{
					...
					{
						return string.Format("{0}: {1} % {2} != 0 ", new object[3] { "showFrequency", totalInterstitialCount, int3.Value });
					}
				}
			}
			...
			{
				return string.Format("{0}: {1} >= {2}", new object[3] { fakeInterstitialCount, int4.Value, "maxShowCountDuringSession" });
			}
			return string.Empty;
		}
	}
