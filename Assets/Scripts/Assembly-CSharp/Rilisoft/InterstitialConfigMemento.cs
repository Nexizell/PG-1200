...
{
	[Serializable]
	public sealed class InterstitialConfigMemento 
	{
		...
		internal double GetTimeoutBetweenShowInMinutes(string category)
		{
			...
			if (Overrides.TryGetValue(category, out value) && value != null)
			{
				return value.TimeoutBetweenShowInMinutes ?? TimeoutBetweenShowInMinutes;
			}
		...
		internal static InterstitialConfigMemento FromDictionary(Dictionary<string, object> dictionary)
		{
			...
			interstitialConfigMemento.TimeoutBetweenShowInMinutes = ParsingHelper.GetDouble(dictionary, "timeoutBetweenShowInMinutes") ?? 15.0;
