...
namespace Rilisoft
{
	internal sealed class AfterMatchInterstitialRunner : FyberInterstitialRunnerBase
	{
		public static string GetReasonToDismissInterstitialAfterMatch(bool winner, double matchDurationInMinutes)
		{
			...
			if (afterMatch == null)
			{
				return string.Format("`{0}` config is `null`", new object[1] { afterMatch.Id });
			}
			...
			bool? flag = (winner ? afterMatch.GetEnabledForWinnerOverride(playerCategory) : afterMatch.GetEnabledForLoserOverride(playerCategory));
			if (flag.HasValue)
			{
				if (!flag.Value)
				{
			...
			else if (!(winner ? afterMatch.EnabledForWinner : afterMatch.EnabledForLoser))
			{
