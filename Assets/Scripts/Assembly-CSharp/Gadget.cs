...
{
	protected SaltedFloatEps _cooldownTime = new SaltedFloatEps(0f);

	protected SaltedFloatEps _durationTime = new SaltedFloatEps(0f);
	...
	public static Gadget Create(GadgetInfo gadgetInfo)
	{
		...
		try
		{
			...
			if (GadgetsInfo.Upgrades["gadget_firecrackers"].Contains(gadgetInfo.Id))
			{
				result = new FirecracketsGadget(gadgetInfo);
			}
