...
public class TurretController_MedStation : TurretController
{
	...
	{
		base.UpdateTurret();
		if (isRun && !isKilled && (!isEnemyTurret || GameConnect.isCOOP) && WeaponManager.sharedManager.myPlayerMoveC != null && nextHealTime < Time.time && (base.transform.position - WeaponManager.sharedManager.myPlayerMoveC.myPlayerTransform.position).sqrMagnitude < healRadius * healRadius)
		{
			...
			{
				WeaponManager.sharedManager.myPlayerMoveC.myPetEngine.AddCurrentHealth(healValue);
			}
