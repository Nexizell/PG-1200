...
public class SkinEditorController : MonoBehaviour
{
	...
	public static BrashMode brashMode;

	public static BrashMode brashModeOld;
	...
	private void Awake()
	{
		...
		characterInterface = UnityEngine.Object.Instantiate(original, Vector3.zero, Quaternion.Euler(0f, 158.15f, 0f)).GetComponent<CharacterInterface>();
	...
	private void Start()
	{
		...
		{
			(saveChangesButton.GetComponent<DialogEscape>() ?? saveChangesButton.gameObject.AddComponent<DialogEscape>()).Context = "Save Skin Changes Dialog";
		...
		{
			(okInSaveSkin.GetComponent<DialogEscape>() ?? okInSaveSkin.gameObject.AddComponent<DialogEscape>()).Context = "Save Skin as... Dialog";
		...
		{
			(yesInLeaveSave.GetComponent<DialogEscape>() ?? yesInLeaveSave.gameObject.AddComponent<DialogEscape>()).Context = "Save Skin Dialog";
		...
		{
			(yesSaveButtonInEdit.GetComponent<DialogEscape>() ?? yesSaveButtonInEdit.gameObject.AddComponent<DialogEscape>()).Context = "Save Cape Dialog";
	...
	private void OpenPresetsWindow()
	{
		...
		if (!presetPreviewInitialized)
		{
			...
			{
				CharacterInterface component = UnityEngine.Object.Instantiate(original, Vector3.zero, Quaternion.identity).GetComponent<CharacterInterface>();
	...
	private void HandleYesSaveButtonInEditClicked(object sender, EventArgs e)
	{
		...
		isEditingPartSkin = false;
		HandleBackButtonClicked(null, (modeEditor == ModeEditor.LogoClan) ? new EditorClosingEventArgs
		{
			ClanLogoSaved = true
		} : null);
	}
	...
	{
		string text = (sender as MonoBehaviour).gameObject.name;
		Debug.Log(text);
		if (text.Equals("Fill"))
		{
	...
	private void SetPartsRect()
	{
		...
		if (modeEditor == ModeEditor.SkinPers)
		{
			...
			rectsPartsInSkin.Add("Head", dictionary);
			...
			rectsPartsInSkin.Add("Foot_left", dictionary2);
			...
			rectsPartsInSkin.Add("Foot_right", dictionary3);
			...
			rectsPartsInSkin.Add("Body", dictionary4);
			...
			rectsPartsInSkin.Add("Arm_right", dictionary5);
			...
			rectsPartsInSkin.Add("Arm_left", dictionary6);
		}
	}
	...
	private void ExitFromScene(EventArgs e = null)
	{
		...
		{
			SkinEditorController.ExitFromSkinEditor((modeEditor == ModeEditor.LogoClan && e != null && e is EditorClosingEventArgs && (e as EditorClosingEventArgs).ClanLogoSaved) ? "SAVED" : newNameSkin);
	...
	static SkinEditorController()
	{
		...
		colorForPaint = new Color(0f, 1f, 0f, 1f);
		colorHistory = new Color[5]
		{
			Color.clear,
			Color.clear,
			Color.clear,
		...
		brashMode = BrashMode.Pencil;
		brashModeOld = BrashMode.Pencil;
		modeEditor = ModeEditor.SkinPers;
		sharedController = null;
		currentSkin = null;
		currentSkinName = null;
		rectsPartsInSkin = new Dictionary<string, Dictionary<string, Rect>>();
		texturesParts = new Dictionary<string, Dictionary<string, Texture2D>>();
		isEditingPartSkin = false;
		isEditingSkin = false;
	}
}
