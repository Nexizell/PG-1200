...
public sealed class BlinkReloadButton : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (isBlink)
		{
			...
			if (timerBlink < 0f)
			{
				...
				{
					blinkObjs[i].color = (isBlinkState ? blinkColor : unBlinkColor);
				}
			}
		}
		if (!isBlink && isBlinkState)
		{
			...
			{
				blinkObjs[j].color = (isBlinkState ? blinkColor : unBlinkColor);
			}
		}
		isBlinkOld = isBlink;
	}
