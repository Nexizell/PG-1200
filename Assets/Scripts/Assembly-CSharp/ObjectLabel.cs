...
public class ObjectLabel : MonoBehaviour
{
    ...
    public UnityEngine.UI.RawImage clanTexture;

    public UnityEngine.UI.Text clanName;
    ...
    private void Start()
    {
        ...
        //base.transform.GetComponent<UnityEngine.UI.RawImage>().pixelInset = new Rect(-75f * koofScreen, -3f * koofScreen, 30f * koofScreen, 30f * koofScreen);
        //base.transform.GetComponent<UnityEngine.UI.Text>().pixelOffset = new Vector2(-45f * koofScreen, 0f);
        base.transform.GetComponent<UnityEngine.UI.Text>().fontSize = Mathf.RoundToInt(20f * koofScreen);
    ...
    private void Update()
    {
        ...
        if ((GameConnect.isSpleef && MiniGamesController.Instance != null && !MiniGamesController.Instance.isGo) || _weaponManager.myPlayer == null)
		{
			ResetTimeShow();
		}
        if ((GameConnect.gameMode == GameConnect.GameMode.TeamFight || GameConnect.gameMode == GameConnect.GameMode.FlagCapture || GameConnect.gameMode == GameConnect.GameMode.CapturePoints) && _weaponManager.myPlayer != null && _weaponManager.myPlayerMoveC != null && target.GetComponent<Player_move_c>() != null && _weaponManager.myPlayerMoveC.myCommand == target.GetComponent<Player_move_c>().myCommand)
        {
            ResetTimeShow();
        }
        try
        {
            cam = currentCamera;
            camTransform = cam.transform;
            UnityEngine.UI.RawImage component = base.transform.GetComponent<UnityEngine.UI.RawImage>();
            if (component == null)
            {
            ...
            if (!isMenu)
            {
                ...
                if (!isSetColor)
                {
                    ...
                    {
                        base.gameObject.GetComponent<UnityEngine.UI.Text>().color = Color.blue;
                        isSetColor = true;
                    }
                    if (component2.myCommand == 2)
                    {
                        base.gameObject.GetComponent<UnityEngine.UI.Text>().color = Color.red;
                        isSetColor = true;
                    }
                }
            ...
            else
            {
                ...
                //base.transform.GetComponent<UnityEngine.UI.Text>().pixelOffset = new Vector2(-85f * koofScreen, 0f);
                //base.transform.GetComponent<UnityEngine.UI.Text>().fontSize = Mathf.RoundToInt(20f * Defs.Coef);
