...
public sealed class DeleteFriend : MonoBehaviour
{
	...
	private bool IsDeletePlayerFromClan(string clanId, string leaderId, string friendId)
	{
		...
		bool num = !string.IsNullOrEmpty(clanId) && !string.IsNullOrEmpty(leaderId);
		bool flag = sharedController.id.Equals(sharedController.clanLeaderID);
		bool flag2 = friendId != null && sharedController.playersInfo.ContainsKey(friendId);
		if (!num || !flag || !flag2)
		{
			return false;
		}
