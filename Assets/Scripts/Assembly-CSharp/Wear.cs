...
public static class Wear
{
	...
	{
		if (showHatTag != null && wear[ShopNGUIController.CategoryNames.HatsCategory].SelectMany((List<string> list) => list).Contains(showHatTag) && showHatTag != "hat_Adamant_3")
		{
			return !wear[ShopNGUIController.CategoryNames.HatsCategory][0].Contains(showHatTag);
		}
		return false;
	}
	...
	public static float MaxArmorForItem(string armorName, int roomTier)
	{
		...
		{
			int num = Math.Min((ExpController.Instance != null) ? ExpController.Instance.OurTier : (ExpController.LevelsForTiers.Length - 1), roomTier);
			bool flag = false;
			...
			if (list != null && list.IndexOf(armorName ?? "") > 3 * (num + 1) - 1)
			{
				armorName = list[3 * (num + 1) - 1];
			}
			armorNum.TryGetValue(armorName ?? "", out value);
		}
	...
	public static int LeagueForWear(string name, ShopNGUIController.CategoryNames category)
	{
		...
		return shopPositionParams.League;
	}
	...
	public static Dictionary<RatingSystem.RatingLeague, List<string>> UnboughtLeagueItemsByLeagues()
	{
		...
		try
		{
			dictionary = dictionary.ToDictionary((KeyValuePair<RatingSystem.RatingLeague, List<string>> kvp) => kvp.Key, (KeyValuePair<RatingSystem.RatingLeague, List<string>> kvp) => kvp.Value.Where((string item) => Storager.getInt(item) == 0).ToList(), RatingSystem.RatingLeagueComparer.Instance);
		}
		catch (Exception ex)
		{
	...
	public static int TierForWear(string w)
	{
		...
		foreach (KeyValuePair<ShopNGUIController.CategoryNames, List<List<string>>> item in wear)
		{
			foreach (List<string> item2 in item.Value)
			{
				if (item2.Contains(w))
				{
					return (item.Key == ShopNGUIController.CategoryNames.MaskCategory) ? (item2.IndexOf(w) * 2) : item2.IndexOf(w);
				}
			}
		}
		return 0;
	...
	{
		if (!(move == null) && !GameConnect.isHunger && !GameConnect.isSurvival && !GameConnect.isCOOP)
		{
	...
	public static void RenewCurArmor(int roomTier)
	{
		...
		{
			curArmor.Add(key, new SaltedFloat((GameConnect.isHunger || GameConnect.isSurvival || GameConnect.isCOOP) ? 0f : MaxArmorForItem(key, roomTier)));
		}
		foreach (string key2 in armorNumTemp.Keys)
		{
			curArmor.Add(key2, new SaltedFloat((GameConnect.isHunger || GameConnect.isSurvival || GameConnect.isCOOP) ? 0f : MaxArmorForItem(key2, roomTier)));
		}
	}
}
