...
public sealed class PremiumAccountScreenController : MonoBehaviour
{
	...
	private bool InitialFreeAvailable
	{
		get
		{
			return Storager.getInt("PremiumInitialFree1Day") == 0;
		}
	}
		...
		for (int i = 0; i < rentButtons.Length; i++)
		{
			foreach (Transform item in rentButtons[i].transform)
			{
				if (item.name.Equals("GemsIcon"))
				{
					...
					item.GetChild(0).GetComponent<UILabel>().text = itemPrice.Price.ToString();
					break;
				}
			}
		}
	...
	{
		Storager.setInt("PremiumInitialFree1Day", 1);
	}
