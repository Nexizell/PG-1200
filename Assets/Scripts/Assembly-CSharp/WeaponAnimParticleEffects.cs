...
public class WeaponAnimParticleEffects : MonoBehaviour
{
	...
	[SerializeField]
	[ReadOnly]
	protected internal WeaponAnimEffectData _currentEffect;

	[ReadOnly]
	[SerializeField]
	protected internal string _lastStartedAnimationName;

	[SerializeField]
	[ReadOnly]
	protected internal string _lastFinishedAnimationName;
	...
	private List<GameObject> _effectObjects
	{
		get
		{
			if (_eo == null)
			{
				...
				{
					ParticleSystem[] particleSystems = array[i].particleSystems;
	...
	private void InitiAnimatonEventForEffect(WeaponAnimEffectData effectData)
	{
		...
		if (!(animationClip == null))
		{
			...
			effectData.animationLength = (effectData.isLoop ? 0f : animationClip.length);
		}
	}
