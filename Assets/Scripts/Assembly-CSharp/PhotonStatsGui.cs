...
public class PhotonStatsGui : MonoBehaviour
{
	...
	public void TrafficStatsWindow(int windowID)
	{
		...
		int num = Mathf.RoundToInt(_time);
		if (num == 0)
		{
			num = 1;
		}
		...
		string text = string.Format("Out|In|Sum:\t{0,4} | {1,4} | {2,4}", new object[3] { trafficStatsGameLevel.TotalOutgoingMessageCount, trafficStatsGameLevel.TotalIncomingMessageCount, trafficStatsGameLevel.TotalMessageCount });
		string text2 = string.Format("{0}sec average:", new object[1] { num });
		string text3 = string.Format("Out|In|Sum:\t{0,4} | {1,4} | {2,4}", new object[3]
		{
		...
		if (buttonsOn)
		{
			...
			if (GUILayout.Button("Reset"))
			{
				...
				_time = 0f;
				PhotonTrafficStatistic.ResetAndLogStatistic();
			}
