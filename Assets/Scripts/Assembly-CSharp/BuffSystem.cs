...
{
	internal sealed class SituationBuffComparer : IEqualityComparer<SituationBuffType>
	{
	...
	internal class BuffParameter
	{
		...
		{
			type = (SituationBuffType)Enum.Parse(typeof(SituationBuffType), Convert.ToString(dictionary["type"]));
	...
	internal class SituationBuff
	{
		...
		public bool isDebuff
		{
			get
			{
				if (!(param.healthBuff < 1f))
				{
					return param.damageBuff < 1f;
				}
				return true;
			}
		}
	...
	internal enum SituationBuffType
	{
		DebuffBeforeGun = 0,
		DebuffAfterGun = 1,
		TierLvlUp = 2,
	...
	public static BuffSystem instance
	{
		get
		{
			if (_instance == null)
			{
				GameObject obj = new GameObject("BuffSystem");
				UnityEngine.Object.DontDestroyOnLoad(obj);
				_instance = obj.AddComponent<BuffSystem>();
			}
			return _instance;
		}
	}
	...
	public float weaponBuffValue
	{
		get
		{
			...
			return Mathf.Clamp(weaponBuff.param.damageBuff, (GetKillrateByInteractions() < 0.8f) ? 1 : 0, (GetKillrateByInteractions() > 2f) ? 1 : 2);
		}
	}
		...
		if (buffsActive)
		{
			...
			for (int i = 0; i < situationBuffs.Count; i++)
			{
				if (string.IsNullOrEmpty(situationBuffs[i].weapon))
				{
					...
					if (currentBuff != null)
					{
						Debug.Log(string.Format("<color=green>Buff active: {0}</color>", new object[1] { currentBuff.param.type.ToString() }));
					}
				}
				else
				{
					...
					if (weaponBuff != null)
					{
						Debug.Log(string.Format("<color=green>Weapon buff active: {0}</color>", new object[1] { weaponBuffValue }));
					}
				}
			}
			switch (status)
			...
			case CheckStatus.NewPlayer:
				...
				{
					healthBuff = ((ExperienceController.sharedController.currentLevel == 1 || ShopNGUIController.NoviceArmorAvailable) ? firstBuffArmor : firstBuffNoArmor);
				}
				break;
	...
	private float GetBuffPercentByKillRate(float value)
	{
		...
		Debug.Log(string.Format("<color=green>Killrate: {0}</color>", new object[1] { num }));
	...
	public void TryLoadConfig()
	{
		...
		Dictionary<string, object> dictionary = Json.Deserialize(Storager.getString("BuffsParam")) as Dictionary<string, object>;
		...
		if (dictionary2.ContainsKey("buffsParams"))
		{
			...
			for (int i = 0; i < list2.Count; i++)
			{
				...
				SituationBuffType key = (SituationBuffType)Enum.Parse(typeof(SituationBuffType), Convert.ToString(dictionary3["type"]));
	...
	private void LoadValues()
	{
		...
		Dictionary<string, object> dictionary = Json.Deserialize(Storager.getString("buffsValues")) as Dictionary<string, object>;
		if (dictionary != null)
		{
			...
			if (dictionary.ContainsKey("buffs"))
			{
				...
				for (int i = 0; i < list.Count; i++)
				{
					...
					string text = (dictionary2.ContainsKey("weapon") ? Convert.ToString(dictionary2["weapon"]) : "");
		...
		List<object> list2 = Json.Deserialize(Storager.getString("buffsPlayerInteractions")) as List<object>;
		if (list2 != null)
		{
	...
	private void SaveValues()
	{
		...
		Storager.setString("buffsValues", Json.Serialize(dictionary));
		SaveInteractions();
	}
	...
	private void SaveInteractions()
	{
		if (interactionsChanged)
		{
			...
			Storager.setString("buffsPlayerInteractions", Json.Serialize(interactions.Select((InteractionType o) => (int)o).ToArray()));
		}
	}
		...
		if (buffsActive && configLoaded)
		{
			...
			if (status != CheckStatus.NewPlayer && status != CheckStatus.OldPlayer)
			{
				if (lastGiveGunTime + GetTimeForGun() < NotificationController.instance.currentPlayTimeMatch)
				{
					...
					{
						AddSituationBuff(SituationBuffType.DebuffBeforeGun);
					}
					else
					{
						GiveTryGunToPlayer();
	...
	public void OnGunTakeOff()
	{
		...
		AddSituationBuff(SituationBuffType.DebuffAfterGun);
	}
	...
	{
		AddSituationBuff(SituationBuffType.TierLvlUp);
	}
	...
	{
		if (weaponBuff != null && !string.IsNullOrEmpty(weapon))
		{
			return weaponBuff.weapon == weapon;
		}
		return false;
	}
