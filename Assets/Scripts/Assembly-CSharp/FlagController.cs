...
public class FlagController : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		GameObject gameObject = Object.Instantiate(original, myBaza.transform.position, myBaza.transform.rotation);
		pedistal = gameObject.GetComponent<FlagPedestalController>();
		_objBazaTexture = Object.Instantiate(Resources.Load("ObjectPictFlag") as GameObject, myBaza.transform.position, myBaza.transform.rotation);
		_objFlagTexture = Object.Instantiate(Resources.Load("ObjectPictFlag") as GameObject, myBaza.transform.position, myBaza.transform.rotation);
	...
	public void SetColor(int _color)
	{
		if (_color != currentColor)
		{
			...
			if (_color > 0)
			{
				_objBazaTexture.GetComponent<ObjectPictFlag>().SetTexture(Resources.Load((_color == 1) ? "blue_base" : "red_base") as Texture2D);
				_objFlagTexture.GetComponent<ObjectPictFlag>().SetTexture(Resources.Load((_color == 1) ? "blue_flag" : "red_flag") as Texture2D);
			}
			else
			{
