...
public class DailyQuestItem : MonoBehaviour
{
	...
	public void Refresh()
	{
		...
		else if (_questInfo != null && _questInfo.Quest != null)
		{
			FillData(Quest.Slot);
		}
	}
	...
	public void FillData(int slot)
	{
		...
		_questInfo = ((slot == -1) ? questProgress.GetRandomQuestInfo() : questProgress.GetActiveQuestInfoBySlot(slot + 1));
		if (Quest == null)
		{
			base.gameObject.SetActive(false);
			return;
		...
		if (Quest.SetActive())
		{
			Dictionary<string, object> eventParams = new Dictionary<string, object> { { "Total", "Get" } };
			...
			Dictionary<string, object> eventParams2 = new Dictionary<string, object> { { "Get", value } };
		...
		progressLabel.text = string.Format("{0}/{1}", new object[2] { Quest.CurrentCount, Quest.RequiredCount });
		progressBar.fillAmount = (float)Quest.CalculateProgress();
		if (Defs.IsDeveloperBuild && GetComponent<UISprite>() != null)
		{
		...
		if (itemInLobby)
		{
			if (_questInfo.Quest != null)
			{
				...
				if (questsButtonLabel != null && rewardButtonLabel != null)
				{
					...
					if (flag)
					{
						DailyQuestsButton component = questsButtonLabel.parent.GetComponent<DailyQuestsButton>();
						if (component != null)
						{
							component.SetUI();
						}
		...
		expObject.SetActive(Quest.Reward.Experience > 0 && (ExperienceController.sharedController.currentLevel != 36 || (Quest.Reward.Coins == 0 && Quest.Reward.Gems == 0)));
	...
	public void OnGetRewardButtonClick()
	{
		...
		{
			Quest.SetRewarded();
			Dictionary<string, object> eventParams = new Dictionary<string, object> { { "Total", "Rewarded" } };
			AnalyticsFacade.SendCustomEvent("Daily Quests", eventParams);
			string value = string.Format(CultureInfo.InvariantCulture, "{0} / {1}", Quest.Id, QuestConstants.GetDifficultyKey(Quest.Difficulty));
			Dictionary<string, object> eventParams2 = new Dictionary<string, object> { { "Quests", value } };
			AnalyticsFacade.SendCustomEvent("Daily Quests", eventParams2);
	...
	private void HandleSkip()
	{
		...
		{
			_questInfo.Skip();
			Dictionary<string, object> eventParams = new Dictionary<string, object> { { "Total", "Skipped" } };
			AnalyticsFacade.SendCustomEvent("Daily Quests", eventParams);
			QuestBase quest = _questInfo.Quest;
			string value = string.Format(CultureInfo.InvariantCulture, "{0} / {1}", quest.Id, QuestConstants.GetDifficultyKey(quest.Difficulty));
			Dictionary<string, object> eventParams2 = new Dictionary<string, object> { { "Skip", value } };
			AnalyticsFacade.SendCustomEvent("Daily Quests", eventParams2);
	...
	public void OnToBattleButtonClick()
	{
		...
		case "killNpcWithWeapon":
			...
			break;
		case "winInMap":
			ConnectScene.selectedMap = GetQuestMap(Quest);
			MainMenuController.sharedController.OnClickMultiplyerButton();
			break;
		case "killWithWeapon":
