...
public class BankViewItem : RealInappBankViewItem
{
	...
	public override bool isX3Item
	{
		set
		{
			...
			{
				discountPercentsLabel.text = string.Format("{0}%", new object[1] { purchaseInfo.Discount });
			}
			UpdateViewBestBuy();
	...
	{
		_bestBuyAnimator = ((bestBuy == null) ? null : bestBuy.GetComponent<Animator>());
		_discountAnimator = ((discountSprite == null) ? null : discountSprite.GetComponent<Animator>());
		if (bonusButtonView != null)
		{
	...
	{
		bool isLoweMemoryDevice = Device.IsLoweMemoryDevice;
	}
	...
	protected virtual string PathToIcon()
	{
		...
		{
			return "Textures/Bank/Coins_Shop_" + (purchaseInfo.Index + 1);
		}
		return "Textures/Bank/Coins_Shop_Gem_" + (purchaseInfo.Index + 1);
	}
