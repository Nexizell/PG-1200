...
public sealed class PauseNGUIController : ControlsSettingsBase
{
	...
	private new void Start()
	{
		...
		int num;
		if ((Defs.touchPressureSupported || Application.isEditor) && !GameConnect.isSpeedrun)
		{
			num = ((!GameConnect.isDeathEscape) ? 1 : 0);
			if (num != 0)
			{
		...
		{
			float num2 = Mathf.Clamp(Defs.Sensitivity, 6f, 19f);
			float num3 = num2 - 6f;
			sensitivitySlider.value = num3 / 13f;
		...
		controlsButton.GetComponent<ButtonHandler>().Clicked += delegate
		{
			if (!InPauseShop())
			{
				...
				swipePanelInSettings.transform.parent.gameObject.SetActive(!GameConnect.isDaterRegim || !GlobalGameController.switchingWeaponSwipe);
				swipePanelInSettings.SetActive(GameConnect.isDaterRegim && GlobalGameController.switchingWeaponSwipe);
